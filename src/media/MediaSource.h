...
/*
1. MediaSource类维护了一个由AVFrame对象构成的视频帧缓冲池，其中mAVFrames数组用于存储视频帧数据，mAVFrameInputQueue用于管理待处理的视频帧，而mAVFrameOutputQueue用于存储已处理的视频帧。

2. 获取已处理的视频帧，从输出队列中取出一个视频帧并返回。putFrame函数用于将新的视频帧放入待处理队列mAVFrameInputQueue中。
3. MediaSource类利用线程池来处理视频帧，当新的视频帧放入待处理队列时，它会创建一个任务（mTask），并将任务添加到线程池中。这样，视频帧的读取和处理可以并发执行。

4. MediaSource类是一个用于管理视频帧的生产者，它负责缓冲视频帧并将它们放入待处理队列，以供后续处理。同时，它还使用线程池来异步处理视频帧，实现了生产者与消费者模式，用于数据采集与处理。这种设计可以提高多线程处理视频帧的效率和并发性。
*/
class AVFrame
{
public:
