...
RtspServer::RtspServer(UsageEnvironment* env, const Ipv4Address& addr) :
    ...
    2. 在server->start的时候Acceptor::listen开启对mSocket的listen，同时把mAcceptIOEvent注册到EventScheduler。当轮询到mAcceptIOEvent触发新连接进来时候，accept在mAcceptIOEvent事件调度触发。
    ...
    4. TcpServer构造函数设置了Acceptor::handleRead接收连接的回调函数，通过多态调用RtspServer::handleNewConnection。

    5. RtspServer::handleNewConnection根据已经建立连接的connfd,创建RtspConnection对象（其中创建一个TcpConnection，构造函数根据Acceptor类accpet接受连接后返回的connfd创建了用于传输的事件mTcpConnIOEvent，设置读写异常回调函数，默认启用只读事件TcpConnection::readCallback，并添加mTcpConnIOEvent到EventScheduler，获取客户端IPmPeerIp，并设置了是否mIsRtpOverTcp。mTcpConnIOEvent事件发生的时候实际就是在传输数据，而触发事件mTriggerEvent是断开连接的。）。

    6. 接着设置单个RtspConnection关闭连接的回调函数RtspServer::disconnectionCallback（在TcpConnection::handleRead处理读事件客户端关闭连接或读取失败及RtspConnection::handleReadBytes进行option解析错误的时候调用该回调函数，把要断开连接的connfd加入到要断开连接的队列mDisconnectionlist中。并添加断开连接的触发事件mTriggerEvent到调度器）
...
void RtspServer::handleNewConnection(int connfd) {
    ...
    /*
    2. 设置单个RtspConnection关闭连接的回调函数，在TcpConnection::handleRead处理读事件客户端关闭连接或读取失败及RtspConnection::handleReadBytes进行option解析错误的时候调用，把要断开连接的connfd加入到要断开连接的队列中，并添加断开连接的触发事件mTriggerEvent。
    */
...
void RtspServer::handleDisconnection(int sockfd){
    ...
    // 要取消连接的描述符加入到队列mDisconnectionlist，建立连接的时候是存在map<int, RtspConnection*> mConnections
...
void RtspServer::handleDisconnectionList(){
    ...
    // 遍历mDisconnectionlist所有要关闭的连接描述符，从存储连接描述符和RtspConnection的map mConnections中找到对应的RtspConnection进行释放，然后删除对应的fd
