...
Acceptor::Acceptor(UsageEnvironment *env, const Ipv4Address &addr)
	...
	5.
	这里处理连接的回调函数在TcpServer的构造函数设置mAcceptor->setNewConnectionCallback(TcpServer::newConnectionCallback,
	this)实际是通过多态调用rtspServer::newConnectionCallback进行处理

	6.
	mAcceptIOEvent添加到epoll是在server->start()调用Acceptor::listen时候发生，因为在listen之后才会有这样的事件发生，所以要晚些加入到调度器
	*/
