...
// 采用非阻塞 I/O 的 LT 模式（Level-Triggered 水平触发模式）， 从accept返回的文件描述符connfd上读取数据。使用 readv 系统调用进行异步IO，支持一次性读取数据到多个缓冲区中。
// 没有处理 EAGAIN 的逻辑，意味着当connfd就绪，应用程序会持续尝试读取数据，直到读取返回 EAGAIN（表示暂时没有数据可读）为止。
...
int Buffer::read(int fd) {
    ...
    // 基于经验和应用场景考虑的
    // 如果当前缓冲区足够大，是有可能容纳即将读取的数据，那么只需使用一个 iovec 结构体，将数据直接读入当前缓冲区即可。在这种情况下，iovcnt 被设置为 1。
    // 如果当前缓冲区的剩余空间不足以容纳即将读取的数据，就需要使用两个 iovec 结构体。一个用于当前缓冲区，另一个用于额外的临时缓冲区 extrabuf。读取的数据会被分散存储在两个缓冲区中，确保不会丢失任何数据。
