...
/*
1. Timer和TimerManager类实现了一个简单的定时器管理系统，用于处理定时事件。Timer类表示单个定时器，TimerManager类负责管理多个定时器。
2. 每个Timer对象都关联了一个TimerEvent，当定时器触发时，相关的事件回调函数将被执行。这允许用户在定时器触发时执行自定义操作。
