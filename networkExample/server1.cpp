...
// 从缓冲区读取数据，可以通过MSG_PEEK指定不把数据从缓冲区移除
// 读取到数据就返回，没数据就阻塞
ssize_t recvPeek(int sockfd, void *buf, ssize_t len){
    while(1){
        // 只用于套接口
...
// 按行读取readline,只用于套接口
ssize_t readLine(int sockfd, void *buf, ssize_t maxlen){
